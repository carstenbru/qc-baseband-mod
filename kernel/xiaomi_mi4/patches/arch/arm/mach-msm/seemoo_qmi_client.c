...
static void svc_arrive(struct work_struct *work)
{
	...
	rc = qmi_connect_to_service(seemoo_clnt, TEST_SERVICE_SVC_ID, TEST_SERVICE_INS_ID, TEST_SERVICE_INS_ID >> 8);
	qmi_connect_to_service(seemoo_clnt_ind, TEST_SERVICE_SVC_ID, TEST_SERVICE_INS_ID, TEST_SERVICE_INS_ID >> 8);
	if (rc < 0) {
...
static int __init seemoo_qmi_init(void)
{
	...
	rc = qmi_svc_event_notifier_register(TEST_SERVICE_SVC_ID, TEST_SERVICE_INS_ID, TEST_SERVICE_INS_ID >> 8, &seemoo_clnt_nb);
	if (rc < 0) {
	...
	if (ret_status | ret_data) {
		...
		qmi_svc_event_notifier_unregister(TEST_SERVICE_SVC_ID, TEST_SERVICE_INS_ID, TEST_SERVICE_INS_ID >> 8, &seemoo_clnt_nb);
		destroy_workqueue(workqueue);
		return -EFAULT;
	}
