...
rnti_active_start_column = rnti_got_inactive_column+1 #first colum with RNTI active time data, to adapt to different data formats (different analyzers used)
rnti_active_classes = 9 #number of classes for RNTI active time
...
plot for [tp=1:n_lines] inp_file matrix using ($1-rnti_active_start_column+tp*shift):($3/sums[tp]):xticlabel((tp == ceil(n_lines/2.0)) ? system("head -2 " . inp_file . " | awk 'BEGIN {FS=\"\t\"}; {print $".(int($1) + 1)."}' | tail -1")[13:] : "") every ::rnti_active_start_column:tp:rnti_active_start_column+rnti_active_classes-1:tp title system("head -". (tp+2) ." " . inp_file . " | awk 'BEGIN {FS=\"\t\"}; {print $1}' | tail -1") with boxes
