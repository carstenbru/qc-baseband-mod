...
int srslte_dci_msg_to_dl_grant(srslte_dci_msg_t *msg, uint16_t msg_rnti,
		uint32_t nof_prb, uint32_t nof_ports, srslte_ra_dl_dci_t *dl_dci,
		srslte_ra_dl_grant_t *grant) {
	int ret = SRSLTE_ERROR_INVALID_INPUTS;

	if (msg != NULL && grant != NULL) {
		ret = SRSLTE_ERROR;
		...
		ret = srslte_dci_msg_unpack_pdsch(msg, dl_dci, nof_prb, nof_ports,
				crc_is_crnti);
		if (ret) {
...
int srslte_dci_rar_to_ul_grant(srslte_dci_rar_grant_t *rar, uint32_t nof_prb,
		uint32_t n_rb_ho, srslte_ra_ul_dci_t *ul_dci, srslte_ra_ul_grant_t *grant) {
	...
	srslte_ra_type2_from_riv(riv, &ul_dci->type2_alloc.L_crb,
			&ul_dci->type2_alloc.RB_start, nof_prb, nof_prb);
...
void srslte_dci_rar_grant_unpack(srslte_dci_rar_grant_t *rar,
...
void srslte_dci_rar_grant_pack(srslte_dci_rar_grant_t *rar,
		uint8_t grant[SRSLTE_RAR_GRANT_LEN]) {
	uint8_t *grant_ptr = grant;
...
void srslte_dci_rar_grant_fprint(FILE *stream, srslte_dci_rar_grant_t *rar) {
	fprintf(stream,
			"RBA: %d, MCS: %d, TPC: %d, Hopping=%s, UL-Delay=%s, CQI=%s\n", rar->rba,
			rar->trunc_mcs, rar->tpc_pusch, rar->hopping_flag ? "yes" : "no",
			rar->ul_delay ? "yes" : "no", rar->cqi_request ? "yes" : "no");
}
...
uint32_t dci_format1_sizeof(uint32_t nof_prb) {
	...
	uint32_t n = (uint32_t) ceilf((float) nof_prb / srslte_ra_type0_P(nof_prb))
			+ 5 + HARQ_PID_LEN + 1 + 2 + 2;
	if (nof_prb > 10) {
		n++;
	}
...
uint32_t dci_format2_sizeof(uint32_t nof_prb, uint32_t nof_ports) {
	uint32_t n = (uint32_t) ceilf((float) nof_prb / srslte_ra_type0_P(nof_prb))
			+ 2 + HARQ_PID_LEN + 1 + 2 * (5 + 1 + 2) + precoding_bits_f2(nof_ports);
	if (nof_prb > 10) {
		n++;
	}
	while (is_ambiguous_size(n)) {
...
uint32_t dci_format2A_sizeof(uint32_t nof_prb, uint32_t nof_ports) {
	uint32_t n = (uint32_t) ceilf((float) nof_prb / srslte_ra_type0_P(nof_prb))
			+ 2 + HARQ_PID_LEN + 1 + 2 * (5 + 1 + 2) + precoding_bits_f2a(nof_ports);
	if (nof_prb > 10) {
		n++;
	}
	while (is_ambiguous_size(n)) {
...
uint32_t srslte_dci_format_sizeof_lut(srslte_dci_format_t format,
		uint32_t nof_prb) {
 ...
 */
int dci_format0_pack(srslte_ra_ul_dci_t *data, srslte_dci_msg_t *msg,
		uint32_t nof_prb) {
	...
	*y++ = 0;  // format differentiation
	...
	if (data->type2_alloc.L_crb) {
		riv = srslte_ra_type2_to_riv(data->type2_alloc.L_crb,
				data->type2_alloc.RB_start, nof_prb);
	} else {
 ...
 * TODO: TPC and cyclic shift for DM RS not implemented
 */
int dci_format0_unpack(srslte_dci_msg_t *msg, srslte_ra_ul_dci_t *data,
		uint32_t nof_prb) {
	...
	srslte_ra_type2_from_riv(riv, &data->type2_alloc.L_crb,
			&data->type2_alloc.RB_start, nof_prb, nof_prb);
...
int dci_format1_pack(srslte_ra_dl_dci_t *data, srslte_dci_msg_t *msg,
		uint32_t nof_prb) {
	...
	case SRSLTE_RA_ALLOC_TYPE1:
		srslte_bit_unpack((uint32_t) data->type1_alloc.rbg_subset, &y,
				(int) ceilf(log2f(P)));
...
int dci_format1_unpack(srslte_dci_msg_t *msg, srslte_ra_dl_dci_t *data,
		uint32_t nof_prb) {
	...
	if (msg->nof_bits
			!= srslte_dci_format_sizeof(SRSLTE_DCI_FORMAT1, nof_prb, 1)) {
 ...
 */
int dci_format1As_pack(srslte_ra_dl_dci_t *data, srslte_dci_msg_t *msg,
		uint32_t nof_prb,
		bool crc_is_crnti) {
	...
	*y++ = data->type2_alloc.mode;  // localized or distributed VRB assignment
	...
	if (data->type2_alloc.mode == SRSLTE_RA_TYPE2_LOC) {
		...
		if (crc_is_crnti && nof_prb > 50) {
			...
			n_vrb_dl = srslte_ra_type2_n_vrb_dl(nof_prb,
	...
	uint32_t nb_gap = 0;
	if (crc_is_crnti && data->type2_alloc.mode == SRSLTE_RA_TYPE2_DIST
			&& nof_prb >= 50) {
		nb_gap = 1;
 ...
 *
 */
int dci_format1As_unpack(srslte_dci_msg_t *msg, srslte_ra_dl_dci_t *data,
		uint32_t nof_prb,
		bool crc_is_crnti) {
	...
	/* Make sure it's a SRSLTE_DCI_FORMAT0 message */
	if (msg->nof_bits
			!= srslte_dci_format_sizeof(SRSLTE_DCI_FORMAT1A, nof_prb, 1)) {
	...
	if (data->type2_alloc.mode == SRSLTE_RA_TYPE2_LOC) {
		...
		nof_vrb = srslte_ra_type2_n_vrb_dl(nof_prb,
				data->type2_alloc.n_gap == SRSLTE_RA_TYPE2_NG1);
	}
	uint32_t riv = srslte_bit_pack(&y, riv_nbits(nof_prb) - nb_gap);
	srslte_ra_type2_from_riv(riv, &data->type2_alloc.L_crb,
			&data->type2_alloc.RB_start, nof_prb, nof_vrb);
	data->type2_alloc.riv = riv;
...
int dci_format1B_unpack(srslte_dci_msg_t *msg, srslte_ra_dl_dci_t *data,
 ...
 *
 */
int dci_format1Cs_pack(srslte_ra_dl_dci_t *data, srslte_dci_msg_t *msg,
		uint32_t nof_prb) {
	...
	uint32_t n_step = srslte_ra_type2_n_rb_step(nof_prb);
	uint32_t n_vrb_dl = srslte_ra_type2_n_vrb_dl(nof_prb,
			data->type2_alloc.n_gap == SRSLTE_RA_TYPE2_NG1);
...
int dci_format1Cs_unpack(srslte_dci_msg_t *msg, srslte_ra_dl_dci_t *data,
		uint32_t nof_prb) {
	uint32_t L_p, RB_p;
	...
	if (msg->nof_bits
			!= srslte_dci_format_sizeof(SRSLTE_DCI_FORMAT1C, nof_prb, 1)) {
...
int dci_format1D_unpack(srslte_dci_msg_t *msg, srslte_ra_dl_dci_t *data,
		uint32_t nof_prb, uint32_t nof_ports) {
...
int dci_format2AB_pack(srslte_ra_dl_dci_t *data, srslte_dci_msg_t *msg,
		uint32_t nof_prb, uint32_t nof_ports) {
...
int dci_format2AB_unpack(srslte_dci_msg_t *msg, srslte_ra_dl_dci_t *data,
		uint32_t nof_prb, uint32_t nof_ports) {
	...
	default:
		fprintf(stderr,
				"Format2 accepts type0 or type1 resource allocation only\n");
		return SRSLTE_ERROR;
...
int srslte_dci_msg_pack_pdsch(srslte_ra_dl_dci_t *data,
		srslte_dci_format_t format, srslte_dci_msg_t *msg, uint32_t nof_prb,
		uint32_t nof_ports,
		bool crc_is_crnti) {
	msg->format = format;
	switch (format) {
...
int srslte_dci_msg_pack_pusch(srslte_ra_ul_dci_t *data, srslte_dci_msg_t *msg,
		uint32_t nof_prb) {
	return dci_format0_pack(data, msg, nof_prb);
}

int srslte_dci_msg_unpack_pusch(srslte_dci_msg_t *msg, srslte_ra_ul_dci_t *data,
		uint32_t nof_prb) {
...
int srslte_dci_msg_get_type(srslte_dci_msg_t *msg, srslte_dci_msg_type_t *type,
		uint32_t nof_prb, uint16_t msg_rnti) {
